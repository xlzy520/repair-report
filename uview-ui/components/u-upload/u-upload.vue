<template>
	<view class="u-upload" v-if="!disabled">
		...
		>
			...
			<image @tap.stop="doPreviewImage(item.url || item.path, index)" class="u-preview-image" v-if="uploadType === 'img' " :src="item.url || item.path" :mode="imageMode"></image>
			<video :controls="false" :show-center-play-btn="false" @tap.stop="doPreviewImage(item.url || item.path, index)" class="u-preview-image" v-else-if="uploadType === 'video'" :src="item.url || item.path" :mode="imageMode"></video>
		</view>
    ...
    fileList: {
      ...
      handler(val) {
        val.map(value => {
          ...
          const tmp = this.lists.some(val => val.url == value.url)
    ...
    selectFile() {
      ...
      chooseFile
        .then(res => {
          ...
          const tempFiles = this.uploadType === 'video' ? [res] : res.tempFiles
          tempFiles.map((val, index) => {
            ...
            if (val.size > maxSize) {
              ...
              const path = this.uploadType === 'video' ? val.tempFilePath : val.path
