<template>
	<view>
		<view class="upload">
          ...
          <audio v-if="types == 'audio'" class="uploade-audio" :src="current.src" controls></audio>
				</view>
			</block>
              ...
              success: (res) => {
                if(Math.ceil(res.size / 1024) < this.upload_max * 1024){
                  ...
                  uni.uploadFile({
                    ...
                    success: (uploadFileRes) => {
                      this.$emit('successAudio',uploadFileRes)
                    }
                  });
                }else {
                  uni.showModal({
                    title: '提示',
                    content: `第${[...new Set(this.exceeded_list)].join(',')}张音频超出限制${this.upload_max}MB,已过滤`
                  });
                }
              },
              fail: (err) => {
